...
//
  ...
  G4SubtractionSolid* subtraction 
    = new G4SubtractionSolid("Scintillator-sphere",Scinti_box,sphere,0,G4ThreeVector(0,0,center));

  G4LogicalVolume* Scinti_log
    = new G4LogicalVolume(subtraction,Sci,"Tile_logical");
...
//
  ...
  G4VPhysicalVolume* Trigger_phys
  	= new G4PVPlacement(0,G4ThreeVector(7.5*mm,7.5*mm,-6.*mm),Trigger_log,
  			"Trigger",expHall_log,false,0);


// The ESR Design
//
  G4VSolid* ESR_box = new G4Box("ESR_box",fScinti_x + 0.042,fScinti_y + 0.42,fScinti_z + 0.042);

  G4VSolid* ESR_box2 = new G4Box("ESR_box2",fScinti_x + 0.01,fScinti_y + 0.01,fScinti_z + 0.01);

  G4SubtractionSolid* subtraction2 
    = new G4SubtractionSolid("ESR_box-ESR_box2",ESR_box,ESR_box2);

  G4SubtractionSolid* subtraction3 
    = new G4SubtractionSolid("ESR_box-ESR_box2-sphere",subtraction2,sphere,0,G4ThreeVector(0,0,center));

  G4LogicalVolume* Mylar_log
    = new G4LogicalVolume(subtraction3,Mylar,"Mylar_logical");

  G4VPhysicalVolume* Mylar_phys
    = new G4PVPlacement(0,G4ThreeVector(),Mylar_log,"mylar",
  			expHall_log,true,0);
...
//
  ...
  // G4VPhysicalVolume* Epoxy_phys
  //   = new G4PVPlacement(0,G4ThreeVector(-0.09,0,z_epo),Epoxy_log,"Epo",
  // 			sphere_log,false,0);
...
//  G4double efficiency2[num2] = {0.454, 0.473, 0.485, 0.473, 0.454, 0.419, 0.377};
  ...
  G4LogicalSkinSurface* sciSurface1 =
    new G4LogicalSkinSurface("SciSurface1",Scinti_log,ScintToAir);
    
  ScintToAir->SetType(dielectric_dielectric);
  ScintToAir->SetFinish(polished);
  ScintToAir->SetModel(unified);
  ScintToAir->SetSigmaAlpha(sigma_alpha);
  ScintToAir->SetPolish(1);
  ...
  //Mylar
  //
  G4OpticalSurface* surface_Mylar = new G4OpticalSurface("MylarSurface");
  G4LogicalSkinSurface* MylarSurface =
    new G4LogicalSkinSurface("MylarSurface", Mylar_log, surface_Mylar);
  
  surface_Mylar->SetType(dielectric_dielectric);
  surface_Mylar->SetFinish(polished);
  surface_Mylar->SetModel(unified);
  surface_Mylar->SetSigmaAlpha(sigma_alpha);
  surface_Mylar->SetPolish(1);

  G4MaterialPropertiesTable* myST9 = new G4MaterialPropertiesTable();

  myST9->AddProperty("REFLECTIVITY",          ephoton, reflectivity1,   num);
  //  myST9->AddProperty("EFFICIENCY",            ephoton, efficiency,      num);

  surface_Mylar->SetMaterialPropertiesTable(myST9);

  //Epoxy
  //
  G4OpticalSurface* surface_Epoxy = new G4OpticalSurface("EpoxySurface");
  G4LogicalSkinSurface* epoxySurface =
    new G4LogicalSkinSurface("EpoxySurface", Epoxy_log, surface_Epoxy);
...
//---------- visualization attributes ---------------------------------
    ...
    Mylar_log->SetVisAttributes(visAttributes);
    fVisAttributes.push_back(visAttributes);
...
void OpNoviceDetectorConstruction::ConstructSDandField()
{
  ...
  // SetSensitiveDetector("apd_logical", fmppcSD.Get(), true);
    ...
    OpNoviceTriggerSD* triggerSD
      = new OpNoviceTriggerSD("TriggerSD", "TriggerHitsCollection", 1);
    SetSensitiveDetector("Trigger_logical",triggerSD);
