...
G4VPhysicalVolume* musrDetectorConstruction::Construct()  {
  ...
  while (!feof(fSteeringFile)) {
	...
	G4VSolid* solid=NULL;
	G4VSolid* solidtemp1=NULL;
	G4VSolid* solidtemp2=NULL;
	G4VSolid* solidtemp3=NULL;
	...
	if (strcmp(tmpString2,"tubs")==0){
	  ...
	  solid = new G4Box(solidName,x1*mm,x2*mm,x3*mm);
        ...
        else if (strcmp(tmpString2,"box_std_epoxycase")==0){ 
          sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %s %s",
                 name,&x1,&x2,&x3,material,&posx,&posy,&posz,mothersName,rotMatrix);
          sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
          solidName+=name;
	  ...
	  x5 = x2 - 0.05;
	  x6 = x3 - 0.05;
	  G4Box* box_epoxycase = new G4Box("box_epoxycase",x1*mm,x2*mm,x3*mm);
	  G4Box* box_air = new G4Box("box_air",x4*mm,x5*mm,x6*mm);
	  solid = new G4SubtractionSolid(solidName, box_epoxycase, box_air);
        }
	else if (strcmp(tmpString2,"Ubox_stdAir")==0 || strcmp(tmpString2,"Ubox_bttmAir")==0){
          sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %s %s",
                 name,&x1,&x2,&x3,material,&posx,&posy,&posz,mothersName,rotMatrix);
          sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
          solidName+=name;
	  if(strcmp(tmpString2,"Ubox_stdAir")==0){
	    x4 = 0.425;
	    x5 = 0.95;
	    x6 = 0.95;
	    G4Box* box_air = new G4Box("box_air",x1*mm,x2*mm,x3*mm);
	    G4Box* box_epoxy = new G4Box("box_epoxy",x4*mm,x5*mm,x6*mm);
	    G4ThreeVector v_epoxy((x1-x4) * mm , 0 * mm , 0 * mm);
	    solid = new G4SubtractionSolid(solidName, box_air, box_epoxy, 0, v_epoxy);
	    //solid = new G4SubtractionSolid(solidName, box_epoxycase, box_air);
	  }
	  else if(strcmp(tmpString2,"Ubox_bttmAir")==0){
	    x4 = 0.95;
	    x5 = 0.95;
	    x6 = 0.425;
	    G4Box* box_air = new G4Box("box_air",x1*mm,x2*mm,x3*mm);
	    G4Box* box_epoxy = new G4Box("box_epoxy",x4*mm,x5*mm,x6*mm);
	    G4ThreeVector v_epoxy(0 * mm , 0 * mm , (-x3+x6) * mm);
	    solid = new G4SubtractionSolid(solidName, box_air, box_epoxy, 0, v_epoxy);
	    //solid = new G4SubtractionSolid(solidName, box_epoxycase, box_air);
	  }
	}
        else if (strcmp(tmpString2,"Ubox_bttmAir2")==0){
          sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %lf %s %s",
                 name,&x1,&x2,&x3,material,&x7,&posx,&posy,&posz,mothersName,rotMatrix);
          sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*s %*g %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
          solidName+=name;
	  x4 = 0.95;
	  x5 = 0.95;
	  x6 = 0.425;
	  G4Box* box_air = new G4Box("box_air",x1*mm,x2*mm,x3*mm);
	  G4Box* box_epoxy = new G4Box("box_epoxy",x4*mm,x5*mm,x6*mm);
	  G4ThreeVector v_epoxy((x7) * mm , 0 * mm , (-x3+x6) * mm);
	  solid = new G4SubtractionSolid(solidName, box_air, box_epoxy, 0, v_epoxy);
	}
        else if (strcmp(tmpString2,"Ubox_wedgeAir2")==0){
          sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %lf %lf %lf %lf %s %s",
                 name,&x1,&x2,&x3,material,&x4,&x5,&x6,&x10,&posx,&posy,&posz,mothersName,rotMatrix);
          sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*s %*g %*g %*g %*g %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
          solidName+=name;

          if(x6 == 1) // x7 = edge thickness                                                                                                                                                                
            x7 = 0.2;
          else
            x7 = 0.3;

          x8 = x7*(x4 - x5)/(x6 - x7); // x8 = cut edge length                                                                                                                                              
          x9 = x4 + x8; // x9 = total edge length                                                                                                                                                           
          posx = x9 - (x9+x5)/4 -x8 - posx;
	  G4Box* box_air = new G4Box("box_air",x1*mm,x2*mm,x3*mm);
	  G4Box* box_epoxy = new G4Box("box_epoxy",0.95*mm,0.95*mm,0.425*mm);
	  G4ThreeVector v_epoxy((x10) * mm , 0 * mm , (-x3+0.425) * mm);
	  solid = new G4SubtractionSolid(solidName, box_air, box_epoxy, 0, v_epoxy);
	}
	else if (strcmp(tmpString2,"bside_box")==0 || strcmp(tmpString2,"Ubox_wedgeAir")==0 || strcmp(tmpString2,"box_wedge_epoxycase")==0){
          sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %lf %lf %lf %s %s",
                 name,&x1,&x2,&x3,material,&x4,&x5,&x6,&posx,&posy,&posz,mothersName,rotMatrix);
          sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*s %*g %*g %*g %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
          solidName+=name;
	  // x4 = longer wedge x
	  // x5 = shorter wedge x
	  // x6 = scintillator thickness
	  // posx = how far from longter wedge x
	  if(x6 == 1) // x7 = edge thickness
	    x7 = 0.2;
	  else
	    x7 = 0.3;
	  ...
	  if(strcmp(tmpString2,"bside_box")==0){	    
	    solid = new G4Box(solidName,x1*mm,x2*mm,x3*mm);
	  }
	  else if(strcmp(tmpString2,"Ubox_wedgeAir")==0){
	    //	    x4 = 0.95;
	    //	    x5 = 0.95;
	    //	    x6 = 0.425;
	    ...
	    G4Box* box_air = new G4Box("box_air",x1*mm,x2*mm,x3*mm);
	    G4Box* box_epoxy = new G4Box("box_epoxy",0.95*mm,0.95*mm,0.425*mm);
	    G4ThreeVector v_epoxy(0 * mm , 0 * mm , (-x3+0.425) * mm);
	    solid = new G4SubtractionSolid(solidName, box_air, box_epoxy, 0, v_epoxy);
	    //	    solid = new G4Box(solidName,x1*mm,x2*mm,x3*mm);
	  }
	  else if(strcmp(tmpString2,"box_wedge_epoxycase")==0){
	    //	  x4 = x1 - 0.05;
	    //	  x5 = x2 - 0.05;
	    //	  x6 = x3 - 0.05;
	    G4Box* box_epoxycase = new G4Box("box_epoxycase",x1*mm,x2*mm,x3*mm);
	    G4Box* box_air = new G4Box("box_air",(x1-0.05)*mm,(x2-0.05)*mm,(x3-0.05)*mm);
	    solid = new G4SubtractionSolid(solidName, box_epoxycase, box_air);
	  }
	}
	else if (strcmp(tmpString2,"wedge")==0 || strcmp(tmpString2,"wedge_edge")==0){
	  // wedge-shaped scintillor strip
	  //	  sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %s %s",
	  sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %lf %lf %s %lf %lf %lf %s %s",
		 name,&x1,&x2,&x3,&x4,&x5,material,&posx,&posy,&posz,mothersName,rotMatrix);
	  sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
	  ...
	  if(strcmp(tmpString2,"wedge")==0){
	    solid = new G4Trap(solidName,x1*mm,x2*mm,x3*mm,x4*mm);
	  }
	  else if(strcmp(tmpString2,"wedge_edge")==0){
	    x6 = x5*(x3 - x4)/(x2 - x5);
	    x7 = x3 + x6;
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x7*mm,x4*mm);
	    G4Box* box = new G4Box("box",(x6/2)*mm,(x2/2)*mm,(x1/2)*mm);
	    G4ThreeVector tetp1_1((x7-(x7+x4)/4. - x6/2) * mm , 0 * mm , 0 * mm);
	    solid = new G4SubtractionSolid(solidName, trap, box, 0, tetp1_1);
	  }
	}
	else if (strcmp(tmpString2,"Dtprd")==0 || strcmp(tmpString2,"Dtprd_edge")==0 || strcmp(tmpString2,"Dtprd_edge2")==0 ){
	  //	  sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %s %s",
	  sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %lf %lf %lf %s %lf %lf %lf %s %s",
		 name,&x1,&x2,&x3,&x4,&x5,&x8,material,&posx,&posy,&posz,mothersName,rotMatrix);
	  sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
	  solidName+=name;
	  //	  G4cout<<"DEBUG KAMIL: xxx1="<<xxx1<<",  xxx2="<<xxx2<<",  xxx3="<<xxx3<<G4endl;
	  if(strcmp(tmpString2,"Dtprd")==0){	  
	    G4ThreeVector tetp1_1((x4 - (x3 + x4) / 4.) * mm , (+x2 / 2.) * mm , (+x1 / 2.) * mm);
	    G4ThreeVector tetp1_2((x4 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2.) * mm);
	    G4ThreeVector tetp1_3((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2.) * mm);
	    G4ThreeVector tetp1_4((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , 0);
	    G4ThreeVector tetp2_1((x4 - (x3/2 + x4/2) / 2.) * mm , (+x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_2((x4 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_3((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_4((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , 0);
	    
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x3*mm,x4*mm);
	    G4Tet* tet1 = new G4Tet("tet1", tetp1_1, tetp1_2, tetp1_3, tetp1_4);
	    G4Tet* tet2 = new G4Tet("tet2", tetp2_1, tetp2_2, tetp2_3, tetp2_4);	    
	    G4SubtractionSolid* sbtrtemp = new G4SubtractionSolid("sbtrtemp", trap, tet1);
	    solid = new G4SubtractionSolid(solidName, sbtrtemp, tet2);
	  }
	  else if(strcmp(tmpString2,"Dtprd_edge")==0){
	    /*
	    if(x2 == 1)
	      x5 = 0.2;//cut edge thickness z
	    else
	      x5 = 0.3;
	    */
	    // x5 : y direction thickness of edge
	    x6 = x8*(x3 - x4)/(x2 - x8);//
	    x7 = x3 + x6;
	    // x8 : z direction thickness of edge 
	    G4ThreeVector tetp1_1((x4 - (x7 + x4) / 4.) * mm , (+x2 / 2.) * mm , (+x1 / 2.) * mm);
	    G4ThreeVector tetp1_2((x4 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2.) * mm);
	    G4ThreeVector tetp1_3((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2.) * mm);
	    //	    G4ThreeVector tetp1_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , 0);
	    G4ThreeVector tetp1_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-(x1/2.-x5/2.)/(x3-x4)*(x3-x4+x6) + x1/2.) * mm);//
	    G4ThreeVector tetp2_1((x4 - (x7/2 + x4/2) / 2.) * mm , (+x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_2((x4 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_3((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , ((x1/2.-x5/2.)/(x3-x4)*(x3-x4+x6) - x1/2.) * mm);//
	    
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x7*mm,x4*mm);
	    G4Tet* tet1 = new G4Tet("tet1", tetp1_1, tetp1_2, tetp1_3, tetp1_4);
	    G4Tet* tet2 = new G4Tet("tet2", tetp2_1, tetp2_2, tetp2_3, tetp2_4);	    
	    G4SubtractionSolid* sbtrtemp = new G4SubtractionSolid("sbtrtemp", trap, tet1);
	    G4SubtractionSolid* sbtrtemp2 = new G4SubtractionSolid("sbtrtemp2", sbtrtemp, tet2);
	    G4Box* box = new G4Box("box",(x6/2)*mm,(x2/2)*mm,(x1/2)*mm);
	    G4ThreeVector box1_1((x7-(x7+x4)/4. - x6/2) * mm , 0 * mm , 0 * mm);
	    solid = new G4SubtractionSolid(solidName, sbtrtemp2, box, 0, box1_1);
	  }
	  else if(strcmp(tmpString2,"Dtprd_edge2")==0){
	    x6 = x8*(x3 - x4)/(x2 - x8);
	    x7 = x3 + x6;
	    x9 = x5/2 - x6*(x1/2 - x5/2 - x5/2)/(x3 - x4);
	    G4ThreeVector tetp1_1((x4 - (x7 + x4) / 4.) * mm , (+x2 / 2.) * mm , (+x1 / 2. - x5/2.) * mm);
	    G4ThreeVector tetp1_2((x4 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2. - x5/2.) * mm);
	    G4ThreeVector tetp1_3((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2. - x5/2.) * mm);
	    G4ThreeVector tetp1_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , +x9 * mm);
	    G4ThreeVector tetp2_1((x4 - (x7 + x4) / 4.) * mm , (+x2 / 2.) * mm , (-x1 / 2. + x5/2.) * mm);
	    G4ThreeVector tetp2_2((x4 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2. + x5/2.) * mm);
	    G4ThreeVector tetp2_3((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2. + x5/2.) * mm);
	    G4ThreeVector tetp2_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , -x9 * mm);
	    
	    G4Box* box1 = new G4Box("box1",((x7-x4)/2)*mm,(x2/2)*mm,(x5/4)*mm);
	    G4ThreeVector v_box1((x7-(x7+x4)/4. - (x7-x4)/2.) * mm , 0 * mm , (-x1/2. + x5/4.) * mm);
	    G4ThreeVector v_box2((x7-(x7+x4)/4. - (x7-x4)/2.) * mm , 0 * mm , (+x1/2. - x5/4.) * mm);
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x7*mm,x4*mm);
	    G4Tet* tet1 = new G4Tet("tet1", tetp1_1, tetp1_2, tetp1_3, tetp1_4);
	    G4Tet* tet2 = new G4Tet("tet2", tetp2_1, tetp2_2, tetp2_3, tetp2_4);
	    G4SubtractionSolid* sbtrtemp = new G4SubtractionSolid("sbtrtemp", trap, tet1);
	    G4SubtractionSolid* sbtrtemp2 = new G4SubtractionSolid("sbtrtemp2", sbtrtemp, tet2);
	    G4SubtractionSolid* sbtrtemp3 = new G4SubtractionSolid("sbtrtemp3", sbtrtemp2, box1, 0, v_box1);
	    G4SubtractionSolid* sbtrtemp4 = new G4SubtractionSolid("sbtrtemp4", sbtrtemp3, box1, 0, v_box2);
	    G4Box* box = new G4Box("box",(x6/2)*mm,(x2/2)*mm,(x1/2)*mm);
	    G4ThreeVector v_box3((x7-(x7+x4)/4. - x6/2.) * mm , 0 * mm , 0 * mm);
	    solid = new G4SubtractionSolid(solidName, sbtrtemp4, box, 0, v_box3);
	  }
	  //solid = new G4Tet(solidName, tetp2_1, tetp2_2, tetp2_3, tetp2_4);
	}
	else if (strcmp(tmpString2,"Stprd")==0 || strcmp(tmpString2,"Stprd_edge")==0 || strcmp(tmpString2,"Stprd_edge2")==0){ //single-side tapered wedge 
	  //      sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %s %lf %lf %lf %s %s",  
          sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %lf %lf %lf %s %lf %lf %lf %s %s",
                 name,&x1,&x2,&x3,&x4,&x5,&x8,material,&posx,&posy,&posz,mothersName,rotMatrix);
          sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
          solidName+=name;
	    // x5 : y direction edge thickness
	    // x9 : z direction edge thickness
	  ...
	  G4ThreeVector tetp1_1((x4 - (x3 + x4) / 4.) * mm , (+x2 / 2.) * mm , (+x1 / 2.) * mm);
          ...
          G4ThreeVector tetp1_4((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , 0);
	  */
	  if(strcmp(tmpString2,"Stprd")==0){	  
	    G4ThreeVector tetp2_1((x4 - (x3 + x4) / 4.) * mm , (+x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_2((x4 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_3((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_4((x3 - (x3 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2.) * mm);
	    
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x3*mm,x4*mm);
	    G4Tet* tet2 = new G4Tet("tet2", tetp2_1, tetp2_2, tetp2_3, tetp2_4);
	    solid = new G4SubtractionSolid(solidName, trap, tet2);
	  }
	  else if(strcmp(tmpString2,"Stprd_edge")==0){
	    x6 = x8*(x3 - x4)/(x2 - x8);//
	    x7 = x3 + x6;
	    G4ThreeVector tetp2_1((x4 - (x7 + x4) / 4.) * mm , (+x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_2((x4 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    G4ThreeVector tetp2_3((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2.) * mm);
	    //	    G4ThreeVector tetp2_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (+x1 / 2.) * mm);
	    G4ThreeVector tetp2_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2. + (x1-x5)/(x3-x4)*(x3-x4+x6)) * mm);
	    
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x7*mm,x4*mm);
	    G4Tet* tet2 = new G4Tet("tet2", tetp2_1, tetp2_2, tetp2_3, tetp2_4);
	    G4SubtractionSolid* sbtrtemp = new G4SubtractionSolid("sbtrtemp", trap, tet2);
	    G4Box* box = new G4Box("box",(x6/2)*mm,(x2/2)*mm,(x1/2)*mm);
	    G4ThreeVector box1_1((x7-(x7+x4)/4. - x6/2.) * mm , 0 * mm , 0 * mm);
	    solid = new G4SubtractionSolid(solidName, sbtrtemp, box, 0, box1_1);
	  }
	  else if(strcmp(tmpString2,"Stprd_edge2")==0){
	    x6 = x8*(x3 - x4)/(x2 - x8);
	    x7 = x3 + x6;

	    G4ThreeVector tetp2_1((x4 - (x7 + x4) / 4.) * mm , (+x2 / 2.) * mm , (-x1 / 2. + x5) * mm);
	    G4ThreeVector tetp2_2((x4 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2. + x5) * mm);
	    G4ThreeVector tetp2_3((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2. + x5) * mm);
	    G4ThreeVector tetp2_4((x7 - (x7 + x4) / 4.) * mm , (-x2 / 2.) * mm , (-x1 / 2. + x5 + (x1-x5*2.)/(x3-x4)*(x3-x4+x6)) * mm);
	    G4Trap* trap = new G4Trap("trap",x1*mm,x2*mm,x7*mm,x4*mm);
	    G4Tet* tet2 = new G4Tet("tet2", tetp2_1, tetp2_2, tetp2_3, tetp2_4);
	    G4Box* box1 = new G4Box("box1",((x7-x4)/2)*mm,(x2/2)*mm,(x5/2)*mm);
	    G4Box* box2 = new G4Box("box2",(x6/2)*mm,(x2/2)*mm,(x1/2)*mm);
	    G4ThreeVector v_box1((x7-(x7+x4)/4. - (x7-x4)/2) * mm , 0 * mm , (- x1/2 + x5/2) * mm);
	    G4ThreeVector v_box2((x7-(x7+x4)/4. - x6/2) * mm , 0 * mm , 0 * mm);
	    G4SubtractionSolid* sbtrtemp = new G4SubtractionSolid("sbtrtemp", trap, box1, 0, v_box1);
	    G4SubtractionSolid* sbtrtemp2 = new G4SubtractionSolid("sbtrtemp2", sbtrtemp, tet2);
	    solid = new G4SubtractionSolid(solidName, sbtrtemp2, box2, 0, v_box2);
	  }
        }
	...
	if (musrParameters::boolG4OpticalPhotons) {
	  ...
	  optSurfTMP->SetModel(OpticalModel);
          ...
          if(strcmp(finish,"polishedbackpainted")==0 && (strcmp(optSurfaceName,"airBoundary1")==0 || strcmp(optSurfaceName,"airBoundary2")==0) ){
            optSurfTMP->SetSigmaAlpha(0.1);
            G4cout<<"Optical surface = "<<optSurfaceName<<", SigmaAlpha = "<<optSurfTMP->GetSigmaAlpha()<<G4endl;
          }
	  /*
	  if(strcmp(optSurfaceName,"airBoundary1")==0 || strcmp(optSurfaceName,"airBoundary2")==0){
	    optSurfTMP->SetSigmaAlpha(0.1);
	    G4cout<<"SigmaAlpha = "<<optSurfTMP->GetSigmaAlpha()<<G4endl;
	  }
*/
	  /*
	  if(strcmp(optSurfaceName,"airBoundary1")==0){
	    G4cout<<"***Conditional start!***"<<G4endl;
	  optSurfTMP->SetSigmaAlpha(0.1);

	  const G4int NUM =2;
          G4double pp[NUM] = {2.0*eV,4.5*eV};
          ...
          G4MaterialPropertiesTable* optScintMPT = new G4MaterialPropertiesTable();
          optScintMPT->AddProperty("SPECULARLOBECONSTANT",pp,specularlobe,NUM);
          optScintMPT->AddProperty("SPECULARSPIKECONSTANT",pp,specularspike,NUM);
          optScintMPT->AddProperty("BACKSCATTERCONSTANT",pp,backscatter,NUM);
          optScintMPT->AddProperty("REFLECTIVITY",pp,reflectivity,NUM);
          optScintMPT->AddProperty("DIFFUSELOBECONSTANT",pp,diffuselobe,NUM);
          optScintMPT->AddProperty("RINDEX",pp,rindex,NUM);
          optSurfTMP->SetMaterialPropertiesTable(optScintMPT);

	  G4cout<<"*** RINDEX = "<<optScintMPT->GetProperty("RINDEX")<<G4endl;
	  G4cout<<"*** REFLECTIVITY = "<<optScintMPT->GetProperty("REFLECTIVITY")<<G4endl;

	    G4cout<<"***Conditional end!***"<<G4endl;
	  }//if airBoundary1
	  */
	  /*
	    G4cout<<"***parameters***"<<G4endl;
	    G4cout<<"opticalSurfaceName : "<<optSurfaceName<<G4endl;
	    G4cout<<"SigmaAlpha = "<<optSurfTMP->GetSigmaAlpha()<<G4endl;
	    G4cout<<"Finish = "<<optSurfTMP->GetFinish()<<G4endl;
	    G4cout<<"Model = "<<optSurfTMP->GetModel()<<G4endl;
	  */
	    new G4LogicalBorderSurface(optSurfaceName,pPhysVol1,pPhysVol2,optSurfTMP);
	    //	    if(strcmp(optSurfaceName,"airBoundary1")==0)
	    //	      new G4LogicalBorderSurface(optSurfaceName,pPhysVol2,pPhysVol1,optSurfTMP);
...
//--------- Material definitions ---------
 ...
 // Elements required for MCPglass
  ...
  G4Material* epoxy = // Epoxy glass (used in MPPC package)                                                                                                                                                                                                                     
    new G4Material("Epoxy", density=1.85*g/cm3, ncomponents=3);
